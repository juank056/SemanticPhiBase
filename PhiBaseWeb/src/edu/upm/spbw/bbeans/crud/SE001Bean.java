...
/**
 * Bean para el servicio SE001 <br/>
 * SE001: Mantenimiento de los servicios del sistema Cspsersi
 * 
 * @author Juan Camilo
 */
public class SE001Bean extends BaseBean<Cspsersi, String> {
	...
	protected void initServices() {
		// DAO
		objectDao = appContext.getBean(ICspsersiDAO.class);
	...
	protected void loadSubfile() {
		try {
			...
			// Adiciona objetos
			for (Cspsersi sersi : subfileList) {
				// Adiciona objeto
				listDropdown.add(new SelectItem(sersi.getSsicssiak(), sersi
						.getSsinssiaf()));
			}
	...
	protected void initObjects() {
		...
		// Inicia el servicio del Sistema de Trabajo
		object = new Cspsersi();
		// Prepara objeto
	...
	protected boolean validateFields(String action) {
		...
		isError = false;
		// Valida codigo del Servicio
		if (this.isBlanks(object.getSsicssiak())) {
			isError = true;
			this.getErrorMessages().add(
					ApplicationMessages
							.getMessage("USR0008", ApplicationMessages
									.getMessage("se001_codeService")));
		}
		// Valida Nombre del Servicio
		if (this.isBlanks(object.getSsinssiaf())) {
			isError = true;
			this.getErrorMessages().add(
					ApplicationMessages
							.getMessage("USR0008", ApplicationMessages
									.getMessage("se001_nameService")));
		}
		// Valida Descripcion Servicio
		if (this.isBlanks(object.getSsidssiaf())) {
			isError = true;
			this.getErrorMessages().add(
					ApplicationMessages
							.getMessage("USR0008", ApplicationMessages
									.getMessage("se001_descService")));
		}
		// Valida URL Servicio
		if (this.isBlanks(object.getSsidurlaf())) {
			isError = true;
			this.getErrorMessages()
					.add(ApplicationMessages.getMessage("USR0008",
							ApplicationMessages.getMessage("se001_urlService")));
		}
		// Valida Icono Servicio
		if (this.isBlanks(object.getSsiiconaf())) {
			isError = true;
			this.getErrorMessages().add(
					ApplicationMessages
							.getMessage("USR0008", ApplicationMessages
									.getMessage("se001_iconService")));
		}
		// Retorna
		return !isError;
	}
	...
	public void searchRecord(ActionEvent ev) {
		...
		if (!Constants.BLANKS.equals(searchField)) {
			...
			try {
				...
				object = objectDao.findById(searchField);
