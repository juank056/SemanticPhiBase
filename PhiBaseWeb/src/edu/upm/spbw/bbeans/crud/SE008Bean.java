...
/**
 * Bean para el servicio SE008 <br/>
 * SE008: Consulta del Log de Queries Ejecutadas USDLOGQU
 * 
 * @author Juan Camilo
 */
public class SE008Bean extends BaseBean<Usdlogqu, UsdlogquId> {
	...
	protected void loadSubfile() {
		try {
			...
			subfileList = ((IUsdlogquDAO) objectDao).findByDateRange(sinceDate,
			...
			List<Object[]> users = ((IUsdlogquDAO) objectDao).findUserCodes();
			// Recorre lista
	...
	public void searchRecord(ActionEvent ev) {
		...
		if (!Constants.BLANKS.equals(searchField)) {
			...
			try {
				...
				List<Usdlogqu> logqus = ((IUsdlogquDAO) objectDao)
						.findByDateRangeAndUser(this.searchField, sinceDate,
								untilDate, maxRecordsQuery);
				// Si lo encuentra, adiciona en la lista
				if (logqus.size() > 0) {/* Encontrado */
					// Adiciona en la lista
					for (Usdlogqu logqu : logqus) {
						subfileList.add(logqu);
					}
