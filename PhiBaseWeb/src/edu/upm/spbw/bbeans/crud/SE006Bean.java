...
/**
 * Bean para el servicio SE006 <br/>
 * SE006: Consulta del Log de Ingreso de Usuario USDLOGIN
 * 
 * @author Juan Camilo
 */
public class SE006Bean extends BaseBean<Usdlogin, UsdloginId> {
	...
	protected void initServices() {
		// DAO
		objectDao = appContext.getBean(IUsdloginDAO.class);
	...
	protected void loadSubfile() {
		try {
			...
			subfileList = ((IUsdloginDAO) objectDao).findByDateRange(sinceDate,
					untilDate, maxRecordsQuery);
			...
			List<Object[]> users = ((IUsdloginDAO) objectDao).findUserCodes();
			// Recorre lista
	...
	public void searchRecord(ActionEvent ev) {
		...
		if (!Constants.BLANKS.equals(searchField)) {
			...
			try {
				...
				List<Usdlogin> logins = ((IUsdloginDAO) objectDao)
						.findByDateRangeAndUser(this.searchField, sinceDate,
								untilDate, maxRecordsQuery);
				// Si lo encuentra, adiciona en la lista
				if (logins.size() > 0) {/* Encontrado */
					// Adiciona en la lista
					for (Usdlogin login : logins) {
						subfileList.add(login);
					}
