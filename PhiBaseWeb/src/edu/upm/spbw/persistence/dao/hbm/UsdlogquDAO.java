...
/**
 * DAO para manejar la tabla de Usdlogqu
 * 
 * @author Juan Camilo Mesa
 */
public class UsdlogquDAO extends GenericHbmDAO<Usdlogqu, UsdlogquId> implements
		IUsdlogquDAO {
	 ...
	 */
	public void save(Usdlogqu obj) throws DAOException {
		...
		try {
			...
			criteria.add(Restrictions.eq("id.usuemaiak", obj.getId()
					.getUsuemaiak()));
			criteria.add(Restrictions.eq("id.lqufechfk", obj.getId()
					.getLqufechfk()));
			criteria.setProjection(Projections.max("id.lqunsecnk"));
			// Asigna secuencia
			Object zeroValue = criteria.list().get(0);
			int lastSec = zeroValue != null ? ((Integer) zeroValue).intValue()
					: 0;
			// Asigna secuencia
			obj.getId().setLqunsecnk(lastSec + 1);
			// Guarda objeto
	...
	public List<Usdlogqu> findByDateRange(Date sinceDate, Date untilDate,
		...
		crit.add(Restrictions.ge("id.lqufechfk", sinceDate));
		crit.add(Restrictions.le("id.lqufechfk", untilDate));
		// Orders
		List<Order> orders = new ArrayList<>();
		orders.add(Order.desc("id.lqufechfk"));
		orders.add(Order.desc("lquhoratf"));
		// Ejecuta query
	...
	public List<Object[]> findUserCodes() throws DAOException {
		...
		String query = "SELECT DISTINCT log.id.usuemaiak, usu.usunusuaf"
				+ " FROM Usdlogqu log, Usmusuar usu WHERE usu.usuemaiak=log.id.usuemaiak";
		// Ejecuta
